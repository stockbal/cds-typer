...
class SourceFile extends File {
    ...
    constructor(path) {
        ...
        /** @type {Buffer} */
        this.cdsEntitiesType = new Buffer()
        /** @type {Buffer} */
        this.cdsEntitiesProp = new Buffer()
    }
     ...
     *   export const genre = { horror: 'horror' }
    ...
    addInlineEnum(entityCleanName, entityFqName, propertyName, kvs) {
        ...
        const buffer = this.inlineEnums[entityCleanName] ?? (this.inlineEnums[entityCleanName] = new Buffer())
        printEnum(buffer, propertyName, kvs)
    }
     ...
     */
    // REVISIT: Rename, as no classes are generated now
    addClass(clean, fq) {
    ...
    getImports() {
        const buffer = new Buffer()
        // add import for cds
        if (this.cdsEntitiesType.parts.length > 0) {
            buffer.add('import cds from "@sap/cds"')
        }
    ...
    toTypeDefs() {
        ...
        return [
            ...
            this.#getInlineEnums().join(),
            this.aspects.join(), // needs to be before classes
            this.classes.join(),
            this.#getCdsEntitiesType().join(),
            this.#getCdsEntitiesFunction().join(),
    ...
    toJSExports() {
        return (
            [AUTO_GEN_NOTE, 'const cds = require(\'@sap/cds\')'] // boilerplate
                .concat(
                    // FIXME: move stringification of service into own module
                    this.services.names.map(name => `module.exports = { name: '${name}' }`)
                ) // there should be only one
                .concat(this.#getCdsEntitiesForJS())
                .concat(['// events'])
                .concat(this.events.fqs.map(({ fq, name }) => `module.exports.${name} = '${fq}'`))
                .concat(['// actions'])
                .concat(this.operations.names.map(name => `module.exports.${name} = '${name}'`))
                .concat(['// enums'])
                // export namespaces for inline enums first
                .concat(this.enums.data.filter(e => e.name.includes('.')).map(({ name }) => `module.exports.${name.split('.')[0]} = {}`))
                .concat(this.enums.data.map(({ name, kvs }) => stringifyEnumImplementation(name, kvs)))
                .join('\n') + '\n'
        )
    }
    #getCdsEntitiesForJS() {
        if (this.inflections.length === 0) return []
        return [
            'let _entities',
            'module.exports.entities = function() {',
            '  if (_entities) return _entities',
            `  const csn = cds.entities('${this.path.asNamespace()}')`,
            '  _entities = {',
        ]
            .concat(
                this.inflections
                    ...
                    .flatMap(([singular, plural, original]) => {
                        const exports = [`    ${singular}: { is_singular: true, __proto__: csn.${original} },`]
                        if (!/Array<.*>/.test(plural) && plural !== original) {
                            ...
                            exports.push(`    ${plural}: csn.${original},`)
                        }
                        ...
                        if (singular !== original) {
                            ...
                            exports.push(`    ${original}: ${rhs},`)
                        }
                        return exports
                    })
            ) // singular -> plural aliases
            .concat(['  }', '  return _entities', '}'])
    }
    #getInlineEnums() {
        const buffer = new Buffer()
        if (Object.keys(this.inlineEnums).length === 0) return buffer

        Object.entries(this.inlineEnums).forEach(([entity, inlineBuffer]) => {
            buffer.addIndentedBlock(`export namespace ${entity} {`, inlineBuffer, '}')
        })
        return buffer
    }
    #getCdsEntitiesType() {
        const buffer = new Buffer()
        if (this.cdsEntitiesType.parts.length > 0) {
            buffer.add(`// type for cds.entities of namespace ${this.path.parts.join('.')}`)
            buffer.addIndentedBlock('type Entities = {', this.cdsEntitiesType, '}')
        }
        return buffer
    }
    #getCdsEntitiesFunction() {
        const buffer = new Buffer()
        if (this.cdsEntitiesProp.parts.length > 0) {
            buffer.add('let _entities: Entities')
            buffer.add('/**')
            buffer.add(` * @returns {Entities} entities of namespace "${this.path.parts.join('.')}"`)
            buffer.add(' */')
            buffer.addIndentedBlock(
                'export function entities(): Entities {',
                () => {
                    buffer.add('if (_entities) return _entities')
                    buffer.add(`const csn = cds.entities("${this.path.parts.join('.')}")`)
                    buffer.addIndentedBlock('_entities = {', this.cdsEntitiesProp, '}')
                    buffer.add('return _entities')
                },
                '}'
            )
        }
...
class Buffer {
     ...
     * @param {string | string[] | (() => void) | Buffer} part - either a string, string[], Buffer or a function. If it is a string, it is added to the buffer.
    ...
    addIndented(part) {
        ...
        if (typeof part === 'function') {
            ...
            part.parts.forEach(p => {this.add(p)})
        }
        this.outdent()
    }
