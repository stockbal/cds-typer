...
function unrollDraftability(csn) {
    ...
    //     .forEach(([draftFqn]) => {
    //         const activeFqn = draftFqn.substring(0, draftFqn.length - 7)
    //         // REVISIT: csn.extended does not contain auto exposed entities
    //         // const entity = csn.xtended.definitions[draftFqn.substring(0, draftFqn.length - 7)]
    //         const entity = csn.inferred.definitions[activeFqn]
    //         if (entity) {
    //             entity[annotation] = true
    //             if (entity.projection) {
    //                 const targetName = entity.projection.from.ref[0]
    //                 const target = csn.xtended.definitions[targetName] ?? csn.inferred.definitions[targetName]
    //                 target[annotation] = true
    //             }
    //         }
    //     })
    new DraftUnroller().unroll(csn.xtended)
}
