...
class Visitor {
    ...
    /**
     * @param {{xtended: CSN, inferred: CSN, sql: CSN, draftEnabled: string[]}} csn - root CSN
    ...
    #aspectify(fq, entity, buffer, options = {}) {
        ...
        const ancestors = (entity.includes ?? [])
            ...
            .reduce(toAspectIdent, '')

        this.contexts.push({ entity: fq })

        // collect elements from aspects
        const aspectElements = []
        ;(entity.includes ?? []).forEach(parent => {
            const parentDef = this.csn.xtended.definitions[parent]
            if (parentDef) Object.keys(parentDef.elements).forEach(e => {aspectElements.push(e)})
        })
            ...
            () => {
                ...
                for (let [ename, element] of Object.entries(entity.elements ?? {})) {
                    if (element.target && /\.texts?/.test(element.target)) {
                        LOG.warn(
                            `referring to .texts property in ${fq}. This is currently not supported and will be ignored.`
                        )
                        continue
                    }
                    // skip element if already included in aspect
                    // REVISIT: also skip projected elements and only insert composition/associations to projections
                    if (aspectElements.includes(ename)) continue
                    ...
                    if ('target' in element) {
                        ...
                        for (const [kname, originalKeyElement] of this.#keys(element.target)) {
                            if (getMaxCardinality(element) === 1 && typeof element.on !== 'object') {
                                // FIXME: kelement?
                                const foreignKey = `${ename}_${kname}`
                                if (Object.hasOwn(entity.elements, foreignKey)) {
                                    LOG.error(
                                        `Attempting to generate a foreign key reference called '${foreignKey}' in type definition for entity ${fq}. But a property of that name is already defined explicitly. Consider renaming that property.`
                                    )
                                } else {
    ...
    #printEntity(fq, entity) {
        ...
        if (!isType(entity)) {
            ...
            // use array type of singular with additional properties like $count
            buffer.add(`export type ${plural} = Array<${singular}> & {$count?: number}`)

            this.#printCdsEntitiesType(file, fq, entity, singular, plural)
        }
        buffer.add('')
    }

    /**
     * Prints type and property entries for CdsEntities function
     * @param {SourceFile} file - current namespace file
     * @param {string} fq - the 'true' fqn of the entity
     * @param {entity} entity - entity definition to print
     * @param {string} singular - singular name of the entity
     * @param {string} plural - plural name of the entity
     */
    #printCdsEntitiesType(file, fq, entity, singular, plural) {
        const singularEntityProp = `${singular}: __.Constructable<${singular}> & __.singular & __.withName`
        const actions = Object.entries(entity.actions ?? {})
        const isDraftEnabled = this.csn.draftEnabled.includes(fq)
        const addDraftsProperty = (plural = false) => {
            file.cdsEntitiesType.add(`drafts: __.${plural ? 'Array' : ''}Constructable<__.DraftEntity<${singular}>>`)
        }
        if (actions.length || isDraftEnabled) {
            file.cdsEntitiesType.addIndentedBlock(
                singularEntityProp + ' & {',
                () => {
                    if (actions.length) {
                        file.cdsEntitiesType.addIndentedBlock(
                            'actions: {',
                            actions.map(([aname, action]) =>
            ...
            file.cdsEntitiesType.add(singularEntityProp)
        }

        const pluralEntityProp = `${plural}: __.ArrayConstructable<${singular}> & __.withName`
        if (isDraftEnabled) {
            file.cdsEntitiesType.addIndentedBlock(pluralEntityProp + ' & {', addDraftsProperty.bind(this, true), '}')
        } else {
            file.cdsEntitiesType.add(pluralEntityProp)
        }

        // print entries for typed cds.entities
        file.cdsEntitiesProp.add(`${singular}: __.asSingular(csn.${plural}) as unknown as Entities["${singular}"],`)
        file.cdsEntitiesProp.add(`${plural}: csn.${plural} as unknown as Entities["${plural}"],`)
    }
    ...
    #printEvent(fq, event) {
        ...
        buffer.addIndentedBlock(`export type ${entityName} = {`, () => {
