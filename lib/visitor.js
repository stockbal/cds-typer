...
class Visitor {
    ...
    /**
     * @param {{xtended: CSN, inferred: CSN, sql: CSN}} csn - root CSN
    ...
    #aspectify(fq, entity, buffer, options = {}) {
        ...
        buffer.addIndentedBlock(
            ...
            () => {
                ...
                // STATIC PROPERTIES -> convert to properties on CdsEntities types
                // buffer.addIndented(() => {
                //     for (const e of enums) {
                //         buffer.add(`static ${e.name} = ${propertyToInlineEnumName(clean, e.name)}`)
                //         file.addInlineEnum(clean, fq, e.name, csnToEnumPairs(e, { unwrapVals: true }))
    ...
    #staticClassContents(clean, entity) {
        return isDraftEnabled(entity) ? [`drafts: ${clean}`] : []
    }
    ...
    #printEntity(fq, entity) {
        ...
        if (!isType(entity)) {
            ...
            buffer.add(`export type ${plural} = Array<${singular}> & {${additionalProperties.join('\n')}}`)
